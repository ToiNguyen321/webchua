...
class MY_Model extends CI_Model {
	...
	protected function get_list_set_input($input = array())
	{
		...
		// Thêm điều kiện join cho câu truy vấn thông qua biến $input['join'] 
		//(ví dụ $input['join'] = array('bang_join','bang.id' ,'bangb.id')) 
		//Bảng join phải khác bảng select
		if (isset($input['join'][0]) && isset($input['join'][1]) && isset($input['join'][2]))
		{
			$this->db->join($input['join'][0], $input['join'][1]."=". $input['join'][2], 'left');
		}

		if (isset($input['field']))
		{
			$field = implode($input['field'], ",");
			$this->db->select($field);
		}

		//THêm điều kiện not in các key ko lấy;
