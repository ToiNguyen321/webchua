...
(function($)
{
	$(document).ready(function()
	{
		...
		//Ajax Quan/Huyen
		$('#param_id_tinh').change(function() {
			/* Act on the event */
			var id_tinh = $(this).val();

			$.ajax({
				url: admin_url + '/phuong_quan_tinh/quan/' +id_tinh,
				type: 'POST',
				dataType: 'json',
				data: {
				},
			...
			.done(function(res) {
				...
				if(res.length > 0){
					res.forEach(function(index){
					  ...
					  html += '<option value="'+ index.districtid + '">' + index.type + ' ' + index.name+ '</option>';

					});
					
				}else{
					html = '<option value="">Lựa chọn Quận/Huyện</option>';
				}
				$('#param_id_quan').html(html);

			/* Act on the event */
				var id_quan = $('#param_id_quan').val();

				$.ajax({
					url: admin_url + '/phuong_quan_tinh/phuong/' +id_quan,
					type: 'POST',
					dataType: 'json',
					data: {
					},
				...
				.done(function(res) {
					...
					if(res.length > 0){
						res.forEach(function(index){
						  ...
						  html += '<option value="'+ index.wardid + '">' + index.type + ' ' + index.name+ '</option>';

						});
						
					}else{
						html = '<option value="">Lựa chọn xã/phường</option>';
					}
					$('#param_id_phuong').html(html);
		...
		// $('#param_id_quan').change(function() {
		// 	/* Act on the event */
		// 	reload_phuong_quan('#param_id_quan', 'phuong', '#param_id_phuong', 'wardid', 'Lựa chọn Xã/Phường');

		// });


		// //Function load sự kiện change
		// function reload_phuong_quan(param_id_click, name_function, param_id_load_rs, id_res, string_rs_null, function_load_success = null){
		// 	var id = $(param_id_click).val();

		// 	$.ajax({
		// 		url: admin_url + '/phuong_quan_tinh/'+name_function+'/' +id,
		// 		type: 'POST',
		// 		dataType: 'json',
		// 		data: {
		// 		},
		...
		// 			  html += '<option value="'+ index.id_res + '">' + index.type + ' ' + index.name+ '</option>';
		// 			});
					
		// 		}else{
		// 			html = '<option value="">'+string_rs_null+'</option>';
		// 		}
		// 		$(param_id_load_rs).html(html);
		// 		function_load_success;
		// 	});
		// }
